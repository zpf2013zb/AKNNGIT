...
void egtree_load(const char* filename, vector<TreeNode>& EGTree, vector<updateTreeNode>& UTree);// load gtree index from file
...
//vector<int> dijkstra_candidate(int s, vector<int> &cands, vector<Node> &graph, vector<float> &results); // dijkstra search, used for single-source shortest path search WITHIN one gtree leaf node!
