...
void egtree_load(const char* filename, vector<TreeNode>& EGTree, vector<updateTreeNode>& UTree) {
	// FILE_GTREE
	printf("loading egtreeFile\n");
	FILE *fin = fopen(filename, "rb");
	int *buf = new int[NodeNum];
	float *buff = new float[NodeNum];
	int count_borders=0, count_children=0, count_leafnodes=0;
	bool isleaf;
	int father;

	// clear EGTree
	EGTree.clear();
	ofstream tmp("out.txt");
	while (fread(&count_borders, sizeof(int), 1, fin)) {
		TreeNode tn;
		updateTreeNode utn;
		// 初始化utn
		utn.bitmap = 0;
		//vector<operation> deleteSet;
		//deleteSet.clear();
		//vector<operation> insertSet;
		//insertSet.clear();
		//utn.deleteV = deleteSet;
		//utn.insertV = insertSet;
		//vector<int> children;
		//children.clear();
		//utn.children = children;
		utn.father = -1;
		utn.isLeaf = false;
		utn.nOfDelete = 0;
		utn.nOfInsert = 0;
...
vector<int> dijkstra_candidate(int s, vector<int> &cands, vector<Node> &graph, vector<int>& results) {
	// init
	set<int> todo;
	todo.clear();
	todo.insert(cands.begin(), cands.end());
...
void hierarchy_shortest_path_calculation() {
	...
	//printf("%d, %d, %d\n", Nodes[49722].adjnodes.size(), Nodes[49722].adjnodes[0], Nodes[49722].adjnodes[1]);
	...
	graph = Nodes;
	//printf("%d, %d, %d\n", graph[49722].adjnodes.size(), graph[49722].adjnodes[0], graph[49722].adjnodes[1]);
	vector<int> cands;
	vector<int> result;
	result.clear();
		...
		for (int j = 0; j < treenodelevel[i].size(); j++) {
			...
			if (EGTree[tn].isleaf) {
				...
				for (int k = 0; k < EGTree[tn].union_borders.size(); k++) {
					...
					//vector<float> result(sizel, 0);
					result.clear();
					result = dijkstra_candidate(EGTree[tn].union_borders[k], cands, graph);
					//dijkstra_candidate(EGTree[tn].union_borders[k], cands, graph, result);
					//printf("DIJKSTRA...END\n");
				...
				vector<float> vd(size, 0);
				EGTree[tn].mind = vd;
				for (int d = 0; d < EGTree[tn].union_borders.size(); d++) {
					//printf("DIJKSTRA...LEAF=%d BORDER=%d\n", tn, EGTree[tn].union_borders[k] );
					//vector<float> result(csize,0);
					/*if (EGTree[tn].union_borders[d] == 49722) {
						printf("%d, %d, %d\n", graph[49722].adjnodes.size(), graph[49722].adjnodes[0], graph[49722].adjnodes[1]);
					}*/
					result.clear();
					result = dijkstra_candidate(EGTree[tn].union_borders[d], cands, graph);
					//dijkstra_candidate(EGTree[tn].union_borders[d], cands, graph, result);
					//printf("result size %d\n",result.size());
					//if(EGTree[tn].union_borders.size() != result.size()) printf("The dijsktra compute Error!\n");
					// save to map
					for (int p = 0; p < result.size(); p++) {
						int pos = 0;
						if (d <= p) {
						...
						if (EGTree[tn].mind[pos]>0 && EGTree[tn].mind[pos] != result[p]) {
							...
							//printf("The difference %d %d %f %f %f\n", tn, d, minds, res, (minds-res));
 							//printf("The distance compute Error!\n");
						}
						EGTree[tn].mind[pos] = result[p];
						vertex_pairs[EGTree[tn].union_borders[d]][cands[p]] = result[p];
