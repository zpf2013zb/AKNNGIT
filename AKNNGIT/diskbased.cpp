...
map<int, float> getreAdj(int BaseAddr)
{
	FILE* f = reAdjFile;
	int addr = BaseAddr;

	map<int, float> rtValue;
	rtValue.clear();
	int size, nid;
	float dist;
	char* BlockAddr = getFlatBlock(f, addr / BlkLen);
	memcpy(&size, BlockAddr + (addr%BlkLen), sizeof(int));
	addr = addr + sizeof(int);
	for (int i = 0; i < size; i++) {
		BlockAddr = getFlatBlock(f, addr / BlkLen);
		memcpy(&nid, BlockAddr + (addr%BlkLen), sizeof(int));
		addr = addr + sizeof(int);
		BlockAddr = getFlatBlock(f, addr / BlkLen);
		memcpy(&dist, BlockAddr + (addr%BlkLen), sizeof(float));
...
void OpenDiskComm(string fileprefix, int _cachesize)
{
	...
	if (algorithmId < 6) {
		...
		name = fileprefix + "\\reAdjlist";
		reAdjFile = fopen(name.c_str(), "rb+");
		CheckFile(reAdjFile, tmpFileName);		
		AdjFileSize = getFileSize(reAdjFile);
		cout << "reAdjList size is " << AdjFileSize << endl;
	}
